...
export function StorageSettings({
  ...
  const run = async (action: () => Promise<void> | void) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("A ação não pôde ser concluída. Nada anterior foi descartado."));
    } finally {
      setWorking(false);
    }
  };
  ...
  const exportEverythingLocal = async () => {
    ...
    safeJsonDownload("fate-gameplay-toolkit-backup.json", {
      ...
      note: t("Credenciais privadas das Mesas não entram neste arquivo. Exporte o Histórico dentro de cada Mesa."),
    });
  ...
  return (
    <div className="storage-settings">
      ...
      <section className="storage-overview" data-state={report?.state ?? "normal"}>
        <div className="storage-overview-heading"><HardDrive /><div><b>{t("Armazenamento deste dispositivo")}</b><span>{report ? t(`${formatStorageBytes(report.usage)} usados${report.quota ? ` de aproximadamente ${formatStorageBytes(report.quota)}` : ""}`, "" + String(formatStorageBytes(report.usage)) + " used" + String(report.quota ? ` de aproximadamente ${formatStorageBytes(report.quota)}` : "") + "") : t("Medindo…")}</span></div><strong>{report?.state === "critical" ? t("Crítico") : report?.state === "attention" ? t("Atenção") : t("Normal")}</strong></div>
        {report?.quota && <progress max={report.quota} value={report.usage} aria-label={t("Uso estimado do armazenamento local")} />}
        <p>{percentage === null ? t("Este navegador não informa uma quota total. As categorias abaixo ainda mostram o que a ferramenta consegue medir.") : t(`${percentage.toLocaleString(getAppLanguage() === "pt" ? "pt-BR" : "en-US", { maximumFractionDigits: 1 })}% da quota estimada pelo navegador.`, "" + String(percentage.toLocaleString(getAppLanguage() === "pt" ? "pt-BR" : "en-US", { maximumFractionDigits: 1 })) + "% of the browser’s estimated quota.")} {t("A decomposição é aproximada: o navegador inclui índices, caches e sobrecarga interna no total.")}</p>
        <div className="storage-overview-actions"><Button type="button" variant="outline" size="sm" disabled={working} onClick={() => void refresh()}><RefreshCw /> {t("Medir novamente")}</Button>{report?.persisted === false && <Button type="button" variant="outline" size="sm" disabled={working} onClick={() => void run(async () => { const result = await requestPersistentStorage(); toast[result.persisted ? "success" : "info"](result.persisted ? t("O navegador aceitou proteger estes dados contra limpeza automática.") : result.supported ? t("O navegador não concedeu armazenamento persistente. Seus dados continuam utilizáveis e exportáveis.") : t("Este navegador não oferece armazenamento persistente.")); })}><ShieldCheck /> {t("Pedir proteção ao navegador")}</Button>}</div>
      </section>
      ...
      <section className="storage-action-section">
        ...
        <p>{t("O arquivo inclui as imagens das Fichas somente no momento da exportação. Credenciais privadas e o Histórico das Mesas ficam fora; exporte cada Histórico na própria Mesa.")}</p>
      </section>
      ...
      <section className="storage-action-section">
        ...
        {selectedRooms.size > 0 && <div className="storage-safe-actions"><AlertDialog><AlertDialogTrigger asChild><Button type="button" variant="outline"><Eraser /> {t("Esquecer escolhidas")}</Button></AlertDialogTrigger><AlertDialogContent size="sm"><AlertDialogHeader><AlertDialogTitle>{t("Esquecer")} {selectedRooms.size} {t("Mesa(s) neste dispositivo?")}</AlertDialogTitle><AlertDialogDescription>{t("As credenciais locais escolhidas serão removidas, liberando aproximadamente")}{formatStorageBytes(roomDeleteBytes)}{t(". Nada será apagado do servidor. Para excluir uma Mesa e seus arquivos, entre nela como narrador e use “Excluir Mesa inteira”.")}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{t("Cancelar")}</AlertDialogCancel><AlertDialogAction onClick={() => void run(() => { for (const id of selectedRooms) roomStore.forgetRoom(id); setSelectedRooms(new Set()); toast.success(t("Mesas escolhidas esquecidas neste dispositivo.")); })}>{t("Esquecer Mesas")}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div>}
      </section>
      ...
      <section className="storage-action-section">
        ...
        {!activeRoom ? <p>{t("Abra uma Mesa para medir, exportar ou excluir os dados dela no servidor. Nada é carregado em segundo plano enquanto as Mesas estão fechadas.")}</p> : <>
          ...
          {activeRoom.self.role === "gm" && <p className="storage-orphan-help"><b>{t("O que significa “sem registro”?")}</b> {t("É uma cópia deixada por um envio interrompido que não aparece no Histórico. A verificação não apaga arquivos publicados.")}</p>}
          {activeRoom.self.role === "gm" && <div className="storage-history-cleanup"><div><b>{t("Limpeza opcional do Histórico")}</b><span>{t("Hoje ele ocupa")}{formatStorageBytes(activeRoom.storage.history.usedBytes)}{t(". A limpeza pode liberar até esse valor; o servidor mede o total exato apagado.")}</span></div><Select value={historyDays} onValueChange={setHistoryDays}><SelectTrigger aria-label={t("Período de Histórico a preservar")}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="90">{t("Preservar 90 dias")}</SelectItem><SelectItem value="180">{t("Preservar 180 dias")}</SelectItem><SelectItem value="365">{t("Preservar 1 ano")}</SelectItem><SelectItem value="730">{t("Preservar 2 anos")}</SelectItem></SelectContent></Select><AlertDialog><AlertDialogTrigger asChild><Button type="button" variant="outline" className="destructive-outline"><Trash2 /> {t("Limpar parte antiga")}</Button></AlertDialogTrigger><AlertDialogContent size="sm"><AlertDialogHeader><AlertDialogTitle>{t("Apagar o Histórico anterior ao período escolhido?")}</AlertDialogTitle><AlertDialogDescription>{t("Rolagens, notas e referências de regras anteriores a")}{historyDays} {t("dias serão excluídas. Arquivos, participantes e tudo que for mais recente serão preservados. Exporte o Histórico primeiro se quiser conservar uma cópia.")}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{t("Cancelar")}</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => void run(async () => { const result = await roomStore.clearOldHistory(Date.now() - Number(historyDays) * 86_400_000); toast.success(result.removed ? `${result.removed} item(ns) removido(s), liberando cerca de ${formatStorageBytes(result.bytesFreed)}.` : t("Não havia itens antigos nesse período.")); })}>{t("Apagar parte antiga")}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div>}
          {activeFiles.length > 0 && <div className="storage-server-files"><p>{t("Arquivos na página atual do Histórico")}</p>{activeFiles.map((entry) => { const file = entry.data as RoomFileData; const canDelete = activeRoom.self.role === "gm" || entry.actor.id === activeRoom.self.id; return <div key={entry.id}><span><b>{file.name}</b><small>{formatStorageBytes(file.size)}</small></span><Button type="button" variant="ghost" size="sm" onClick={() => void roomStore.downloadFile(entry).catch((error) => toast.error(error instanceof Error ? error.message : t("O arquivo não pôde ser exportado.")))}><Download /> {t("Exportar")}</Button>{canDelete && <AlertDialog><AlertDialogTrigger asChild><Button type="button" variant="ghost" size="icon-sm" aria-label={t(`Excluir ${file.name}`, "Delete " + String(file.name) + "")}><Trash2 /></Button></AlertDialogTrigger><AlertDialogContent size="sm"><AlertDialogHeader><AlertDialogTitle>{t("Excluir “")}{file.name}”?</AlertDialogTitle><AlertDialogDescription>{t("O registro e o Blob real serão removidos. Os outros arquivos, o Histórico, as Fichas e as regras serão preservados.")}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{t("Cancelar")}</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => void run(async () => { const result = await roomStore.deleteFile(entry); toast.success(result.cleanupPending ? t("Registro removido; a exclusão física continuará automaticamente.") : t("Arquivo excluído por completo.")); })}>{t("Excluir arquivo")}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>}</div>; })}</div>}
          {activeRoom.self.role === "gm" && <AlertDialog><AlertDialogTrigger asChild><Button type="button" variant="outline" className="destructive-outline"><Trash2 /> {t("Excluir esta Mesa do servidor")}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{t("Excluir “")}{activeRoom.room.name}{t("” por completo?")}</AlertDialogTitle><AlertDialogDescription>{t("Participantes, Histórico, referências e todos os Blobs desta Mesa serão removidos. Fichas e regras locais permanecem. Exporte antes: esta ação é irreversível.")}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{t("Cancelar")}</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => void run(async () => { const result = await roomStore.deleteCurrentRoom(); toast.success(result.cleanupPending ? t("Mesa excluída; a limpeza física continuará automaticamente.") : t("Mesa e arquivos excluídos por completo.")); })}>{t("Excluir Mesa")}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>}
        </>}
      </section>
      ...
      <section className="storage-action-section compact storage-cleanup-section">
        ...
        <p className="storage-cleanup-intro">{t("São dados auxiliares: podem ser refeitos pela ferramenta ou já têm uma cópia confirmada. Estas ações não apagam suas Fichas, imagens, regras, credenciais de Mesas nem dados do servidor.")}</p>
