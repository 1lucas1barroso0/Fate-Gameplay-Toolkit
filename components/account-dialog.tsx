...
import { ACCOUNT_MIN_PASSWORD_LENGTH } from "@/lib/account-policy";
...
export function accountMessage(code: string) {
  if (code === "account_deleted") return t("Este cadastro foi apagado. Você pode continuar neste dispositivo ou criar outra conta.", "This account was deleted. You can continue on this device or create another account.");
  ...
  if (["invalid_new_password", "PASSWORD_TOO_SHORT", "PASSWORD_TOO_LONG"].includes(code)) return t(`Use uma nova senha com ${ACCOUNT_MIN_PASSWORD_LENGTH} a 128 caracteres.`, `Use a new password with ${ACCOUNT_MIN_PASSWORD_LENGTH} to 128 characters.`);
  if (code === "invalid_credentials") return t("Confira o e-mail e a senha.", "Check your email and password.");
  if (code === "account_busy") return t("Sua conta está abrindo. Aguarde um instante.", "Your account is opening. Give it a moment.");
  ...
  if (code === "local_save_failed") return t("Não consegui guardar as últimas mudanças neste dispositivo. Exporte uma cópia antes de fechar.", "Your latest changes could not be saved on this device. Export a copy before closing.");
  if (["sync_before_logout", "sync_busy"].includes(code)) return t("Há mudanças chegando de outra sessão. Tente sincronizar de novo em instantes.", "Changes are coming from another session. Try syncing again in a moment.");
  return t("Não foi possível conectar agora. Suas mudanças continuam neste dispositivo. Tente novamente quando a conexão voltar.", "Could not connect right now. Your changes are still on this device. Try again when your connection is back.");
}
...
export function AccountDialog(props: Props) {
  ...
  const run = async (action: () => Promise<unknown>) => {
    ...
    catch (error) { setFailure(accountMessage(error instanceof DOMException && error.name === "QuotaExceededError" ? "local_save_failed" : error instanceof Error ? error.message : "")); }
  ...
  const onOpenChange = (open: boolean) => {
    if (busy) return;
    if (!open && props.recoveryKey) { setMessage(t("Guarde sua chave e toque em Já guardei para continuar.", "Save your key and select I saved it to continue.")); return; }
  ...
  return <Dialog open={props.open} onOpenChange={onOpenChange}>
    <DialogContent className="account-dialog" showCloseButton={!busy}>
      <DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>
        {props.user ? props.user.email : t("Suas Fichas e Mesas com você, em qualquer dispositivo.", "Your sheets and tables with you, on any device.")}
      ...
      {props.user ? <>
        {mode === "delete" ? <form className="account-form" onSubmit={event => { event.preventDefault(); void run(async () => {
          ...
          <p>{t("Não dá para desfazer. Os dados usados só neste dispositivo ficam aqui.", "This cannot be undone. Data used only on this device stays here.")}</p>
          ...
          else { await accountRequest("/api/auth/change-password", { method: "POST", body: JSON.stringify({ currentPassword: password, newPassword: nextPassword }) }); props.setRecoveryKey(""); setMessage(t("Senha alterada. As outras sessões foram encerradas. Crie uma nova chave de recuperação.", "Password changed. Other sessions have been signed out. Create a new recovery key.")); }
          setMode("login");
        }); }}>
          <p>{mode === "key" ? t("A nova chave substitui a anterior. Confirme sua senha para criá-la.", "The new key replaces the previous one. Confirm your password to create it.") : t("Use pelo menos 15 caracteres na nova senha.", "Use at least 15 characters in your new password.")}</p>
          ...
          {mode === "password" ? <><label htmlFor={formId + "-new"}>{t("Nova senha", "New password")}</label><Input id={formId + "-new"} type="password" autoComplete="new-password" required minLength={ACCOUNT_MIN_PASSWORD_LENGTH} maxLength={128} value={nextPassword} onChange={e => setNextPassword(e.target.value)} /></> : null}
          ...
          {props.status === "conflict" ? <div className="account-conflict"><p>{t("A mesma informação foi alterada aqui e em outro dispositivo. Qual versão você quer manter nesses pontos?", "The same information changed here and on another device. Which version should we keep for those changes?")}</p><Button disabled={busy} onClick={() => void run(() => props.resolve("local"))}>{t("Manter a deste dispositivo", "Keep this device’s version")}</Button><Button variant="outline" disabled={busy} onClick={() => void run(() => props.resolve("remote"))}>{t("Manter a da conta", "Keep the account’s version")}</Button></div> : null}
          ...
          <p className="account-hint">{t("Sair mantém seus dados. Mudanças ainda não enviadas ficam neste dispositivo até você entrar novamente.", "Signing out keeps your data. Changes not yet uploaded stay on this device until you sign in again.")}</p>
      ...
      </> : !props.available ? <><p>{t("O cadastro está indisponível no momento. Você pode continuar usando neste dispositivo.", "Accounts are unavailable right now. You can keep using Fate on this device.")}</p><Button onClick={() => onOpenChange(false)}>{t("Continuar neste dispositivo", "Continue on this device")}</Button></> : <form className="account-form" onSubmit={event => {
        event.preventDefault(); const data = new FormData(event.currentTarget);
        void run(async () => {
          if (mode !== "login" && password !== nextPassword) { setFailure(t("As senhas precisam ser iguais.", "The passwords need to match.")); return; }
          ...
          else { await props.authenticate(mode === "signup" ? "signup" : "login", { email: String(data.get("email")), name: String(data.get("name") ?? ""), password, importDevice: data.get("importDevice") === "on" }); }
        });
      }}>
        ...
        <label htmlFor={formId + "-password"}>{mode === "recover" ? t("Nova senha", "New password") : t("Senha", "Password")}</label><Input id={formId + "-password"} type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} required minLength={mode === "login" ? 1 : ACCOUNT_MIN_PASSWORD_LENGTH} maxLength={128} value={password} onChange={e => setPassword(e.target.value)} />
        {mode !== "login" ? <p className="account-hint">{t("Pelo menos 15 caracteres. Uma frase longa funciona bem.", "At least 15 characters. A long phrase works well.")}</p> : null}
        {mode === "signup" || mode === "recover" ? <><label htmlFor={formId + "-confirm"}>{t("Repita a senha", "Repeat your password")}</label><Input id={formId + "-confirm"} type="password" autoComplete="new-password" required minLength={ACCOUNT_MIN_PASSWORD_LENGTH} maxLength={128} value={nextPassword} onChange={e => setNextPassword(e.target.value)} /></> : null}
        {mode !== "recover" ? <label className="account-checkbox"><input type="checkbox" name="importDevice" />{t("Levar os dados deste dispositivo para a conta", "Bring this device’s data into the account")}</label> : null}
