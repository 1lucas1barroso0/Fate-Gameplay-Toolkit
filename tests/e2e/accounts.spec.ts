...
test("optional accounts work across devices; signing out keeps data and deletion revokes the other device", async ({ browser, context, page }, info) => {
  ...
  const guest = createCharacter("Ficha só deste aparelho");
  ...
  try {
    ...
    await dialog.getByLabel("Levar os dados deste aparelho para a conta", { exact: true }).check();
    ...
    await second.locator(".sheet-name-field").fill("Mudança no segundo aparelho"); await sync(second); await sync(page);
    await expect(page.locator(".sheet-name-field")).toHaveValue("Mudança no segundo aparelho");
    await page.getByRole("button", { name: "Minha conta", exact: true }).click();
